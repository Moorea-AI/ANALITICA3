...
def img2data2(path, width=100):
    ...
    for imagePath in (list_labels): ### recorre cada carpeta de la ruta ingresada
        ...
        files_list_ = files_list + files_list_
        for item in tqdm(files_list): ### le pone contador a la lista: tqdm
            ...
            if file[-1] =='g': ### verificar que se imágen extensión jpg o jpeg
                ...
                l = imagePath.split('\\')[2] ### identifica en que carpeta
                if l == 'NonDemented':  ### y le pone la etiqueata
                    labels.append([0])
                elif l == 'Demented':
                    labels.append([1])
    return files_list_, rawImgs, labels
